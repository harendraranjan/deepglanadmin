...
export async function createStaff(body) {
  try { const { data } = await api.post("/create", body); return ok(data); }
  catch (e) { return fail(e); }
}
export async function getAllStaff({ page = 1, q = "" } = {}) {
  try { const { data } = await api.get("/staff", { params: { page, q } }); return ok(data); }
  catch (e) { return fail(e); }
}
export async function getStaffByCode(code) {
  try { const { data } = await api.get(`/staff/${encodeURIComponent(code)}`); return ok(data); }
  catch (e) { return fail(e); }
}
export async function updateStaff(id, body) {
  try { const { data } = await api.patch(`/staff/${id}`, body); return ok(data); }
  catch (e) { return fail(e); }
}

/** ========= ATTENDANCE ========= */
export async function attendanceCheckIn() {
  try { const { data } = await api.post("/staff/attendance/check-in"); return ok(data); }
  catch (e) { return fail(e); }
}
export async function attendanceCheckOut() {
  try { const { data } = await api.post("/staff/attendance/check-out"); return ok(data); }
  catch (e) { return fail(e); }
}
/** Backend: GET /staff/attendance/me?month=YYYY-MM
 *  Expected: { summary: {presentDays, absentDays, totalHours}, logs: [...] }
 *  Flexible handling below.
 */
export async function getMyAttendance({ month } = {}) {
  try { const { data } = await api.get("/staff/attendance/me", { params: { month } }); return ok(data); }
  catch (e) { return fail(e); }
}

/** ========= BUYERS & ORDERS ========= */
export async function getMyBuyers({ page = 1, q = "" } = {}) {
  try { const { data } = await api.get("/staff/my-buyers", { params: { page, q } }); return ok(data); }
  catch (e) { return fail(e); }
}
export async function getOrders({ page = 1, q = "", status } = {}) {
  try { const { data } = await api.get("/staff/orders", { params: { page, q, status } }); return ok(data); }
  catch (e) { return fail(e); }
}
export async function getOrdersCount() {
  try { const { data } = await api.get("/staff/orders/count"); return ok(data); }
  catch (e) { return fail(e); }
}
export async function markReadyToDispatch({ id, courier, awb }) {
  try { const { data } = await api.patch(`/staff/orders/${id}/ready-to-dispatch`, { courier, awb }); return ok(data); }
  catch (e) { return fail(e); }
}
